import { getAlbumsDetails } from "../services/api";
import { navigate } from "../routers/router";
export const initAlbumsDetails = async (slug) => {
  const container = document.getElementById("albumsDetails");

  container.innerHTML = `
         <div class="px-10 py-8 text-white">
            <p class="text-neutral-400">Đang tải album...</p>
        </div>
    `;

  try {
    const albumDetail = await getAlbumsDetails(slug);
        ...
        <div class="flex justify-center">
            <section class="flex gap-12 px-10 py-10">
                ...
                <p class="text-neutral-400 text-sm mb-2">
                    ${albumDetail.songCount} bài hát • ${formatTotalDuration(
                    albumDetail.duration
                    )}
                </p>

                <p class="text-neutral-400 text-sm">
                    ${albumDetail.popularity} lượt nghe
                </p>

                <p class="text-neutral-400 text-sm mt-1">
                    Loại album: ${albumDetail.albumType}
                </p>
